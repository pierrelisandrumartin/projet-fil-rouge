    ...
    <div className="text-white">
      ...
      <div className="flex justify-center mb-8">
        <div className="flex items-center bg-[#1A1D27] rounded-full px-4 py-2 w-full max-w-xl">
          <span className="text-[#9A9AB0] mr-2">🔍</span>
          <input
            type="search"
            id="site-search"
            name="q"
      ...
      {/* Chips catégories */}
      <ul className="flex gap-2 mb-10 flex-wrap">
        {["All", "Manga", "Webtoon", "Manhwa", "Light Novel", "Comics", "BD"].map((cat) => (
          <li key={cat}>
            <button className={`px-4 py-1 rounded-full text-sm border transition-colors
              ${cat === "All"
          ...
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex items-center gap-4 bg-[#1A1D27] rounded-lg p-3">
              ...
              <div className="shrink-0 w-32">
                ...
                <div className="w-full h-1.5 bg-[#252836] rounded-full">
